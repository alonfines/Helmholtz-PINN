...
from model_seq2seq import HelmholtzPINN, set_seed, SEED
from dataset_seq2seq import HelmholtzDataset, custom_collate
from utils_seq2seq import config
...
seq_num = args.seq_num if args.seq_num == 'full' else int(args.seq_num)

config["parameters"]["seq_num"] = seq_num
...
x_train[:, 0] = x_train[:, 0]*config["parameters"]["L_x"]  # Scale x-coordinates to the domain [0, L_x]

if seq_num == 'full':
    x_train[:, 1] = x_train[:, 1]*config["parameters"]["z"]  # Scale y-coordinates to the domain [0, z]
else:
    x_train[:, 1] = dt*(x_train[:,1] + seq_num-1)  # Scale y-coordinates to the domain [(seq_num-1)*dt, seq_num*dt]
...
if seq_num == 'full':
    checkpoint_path = os.path.join(current_dir, checkpoint_dir)+f"/helmholtz_checkpointfull.ckpt"
else:
    checkpoint_path = os.path.join(current_dir, checkpoint_dir)+f"/helmholtz_checkpoint{seq_num}.ckpt"
    
checkpoint_callback = ModelCheckpoint(
    dirpath=checkpoint_dir,
    filename=f'helmholtz_checkpoint{seq_num}',
    save_top_k=1,
    save_last=False,
    mode='min',
    monitor='watch_loss'
...
trainer = pl.Trainer(
    max_epochs=config["parameters"]["epochs"] if seq_num != 'full' else 1,
