...
def get_args():
    ...
    parser.add_argument('--k_squared', type=int, required=False, default=0,
...
# Load config
config_path = os.path.join(current_dir, 'config_curriculum.yaml')
...
def load_fem_data(config, xx, yy):
    ...
    current_dir = os.path.dirname(__file__)
    data_path = os.path.join(current_dir, config["paths"]["grid_data"])
...
def plot_solutions(xx, yy, pinn_sol_amp, fem_amp, config, plots_dir):
    ...
    vmin_real = min(pinn_sol_amp.min(), fem_amp.min())
    vmax_real = max(pinn_sol_amp.max(), fem_amp.max())
    

    
    # Plot real components
    c1 = axes[0].contourf(xx, yy, pinn_sol_amp, levels=50, cmap='jet', 
                           vmin=vmin_real, vmax=vmax_real)
    axes[0].set_title(f'PINN (k^2={config["parameters"]["k_squared"]})')
    
    c2 = axes[1].contourf(xx, yy, fem_amp, levels=50, cmap='jet',
                           vmin=vmin_real, vmax=vmax_real)
    ...
    real_mse = np.sqrt(np.mean((pinn_sol_amp - fem_amp)**2))
...
def load_boundary_conditions(seq_num):
    ...
    bc_path = os.path.join(current_dir, config["paths"]["bc_dir"] + f'/bc{seq_num}.csv')
    data = pd.read_csv(bc_path)
    
    # Convert to torch tensors
    boundary_points = torch.tensor(data[['x', 'z']].values, dtype=torch.float32)
    u = torch.tensor(data['u'].values, dtype=torch.float32)
    
    return boundary_points, u