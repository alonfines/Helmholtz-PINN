...
import yaml
from utils_curriculum import load_boundary_conditions, config, SEED, set_seed, get_args
...
args = get_args()
config["parameters"]["k_squared"] = args.k_squared
...
class HelmholtzPINN(pl.LightningModule):
    ...
    def boundary_loss(self):
        ...
        l2_ibc_loss = torch.mean((pred_bc.squeeze() - u_bc) ** 2)
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='min',
            factor=config['parameters']['factor'],
            patience=config['parameters']['patience'],
            min_lr=config['parameters']['min_lr'],
            verbose=True
        )
